    ...
    <section id="what-we-offer" className="py-20">
      <div className="container mx-auto px-4 max-w-6xl">
        <header className="mb-12 text-center">
          <h3 className="text-4xl font-bold text-gray-900 mb-6 uppercase tracking-wide">{content.whatWeOffer.title}</h3>
          <p className="text-xl text-gray-700 max-w-3xl mx-auto">{content.whatWeOffer.description}</p>
        </header>
        <div className="grid md:grid-cols-2 gap-10">
          {content.whatWeOffer.items.map((item, idx) => (
            <div key={idx} className="bg-white rounded-2xl shadow-lg flex flex-col items-center text-center p-8 hover:shadow-2xl transition">
              ...
              <h4 className="text-xl font-semibold mb-2 text-gray-900 max-w-md mx-auto">{item.title}</h4>
              <p className="text-gray-700 text-base max-w-md mx-auto">{item.text}</p>
            </div>
          ))}
        </div>
      </div>
