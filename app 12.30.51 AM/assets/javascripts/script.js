...
"use strict";
    $(window).on('scroll load', function() {
        if ($(window).scrollTop() > 49) {
            $(".fixed-nav-top").addClass("top-nav-collapse");
            $(".fixed-nav-top").find('.logo-image').removeClass('hidden')
        } else {
            $(".fixed-nav-top").removeClass("top-nav-collapse");
            $(".fixed-nav-top").find('.logo-image').addClass('hidden')
        }
    });
    (function () {
    var userAgent = navigator.userAgent.toLowerCase(), initialDate = new Date(), $document = $(document),
        ...
        isDesktop = $html.hasClass("desktop"), isFirefox = navigator.userAgent.toLowerCase().indexOf('firefox') > -1,
    ...
    $(function () {
        ...
        if (plugins.circleProgress.length) {
            ...
            for (i = 0; i < plugins.circleProgress.length; i++) {
                ;(function () {
                    var proto = $.circleProgress.defaults, originalDrawEmptyArc = proto.drawEmptyArc;
                    proto.emptyThickness = 5;
                    proto.drawEmptyArc = function (v) {
                        var oldGetThickness = this.getThickness, oldThickness = this.getThickness(),
                            emptyThickness = this.emptyThickness || _oldThickness.call(this), oldRadius = this.radius,
            ...
            for (i = 0; i < plugins.bookingCalendar.length; i++) {
                ...
                for (i = 0; i < temp.length; i++) {
                    ...
                    $this.on("click", i, function (event) {
                        ...
                        var tableEvents = $('.rdc-table_events'), ch = tableEvents.outerHeight(),
                            currentEvent = $(this).children('.rdc-table_events'),
                            eventCell = $('#calendarEvent' + event.data), animationDuration = 250;
        ...
        bootstrapCollapse.collapse({toggle: false});
        ...
        if (plugins.countDown.length) {
            ...
            for (i = 0; i < plugins.countDown.length; i++) {
                var countDownItem = plugins.countDown[i], d = new Date(),
